<template>
  <div class="c666">
    ...
    <el-popover ref="popinput" v-model="popInput">
      <div>选择一个类型：</div>
      <el-button-group>
        <el-button size="mini" type="primary" plain @click="addItem('input', {subtype:'text'})">普通文本</el-button>
        <el-button
          size="mini"
          type="primary"
          plain
    ...
    <el-tag v-popover:popinput class="item">
    ...
    <el-popover ref="popradio" v-model="popRadio">
      ...
      <el-button-group>
        ...
        >从服务器获取</el-button>
        <el-button size="mini" type="primary" plain @click="addItem('radio', {options:[]})">自定义</el-button>
      </el-button-group>
    </el-popover>
    <el-tag v-popover:popradio class="item">
    ...
    <el-popover ref="popcheckbox" v-model="popCheckbox">
      ...
      <el-button-group>
        ...
        >从服务器获取</el-button>
        <el-button size="mini" type="primary" plain @click="addItem('checkbox',{options:[]})">自定义</el-button>
      </el-button-group>
    </el-popover>
    <el-tag v-popover:popcheckbox class="item">
    ...
    <el-tag v-popover:popselect class="item">
    ...
    <el-tag v-popover:popdate class="item">
    ...
    <el-popover ref="popcascader" v-model="popCascader">
      ...
      <el-button-group>
        ...
        >从服务器获取</el-button>
        <el-button size="mini" type="primary" plain @click="$message('暂不支持')">自定义</el-button>
      </el-button-group>
    </el-popover>
    <el-tag v-popover:popcascader class="item">
    ...
    <!-- 富文本 -->
    <el-tag class="item" @click.native="addItem('rate')">
      <icon-svg icon="richtext"></icon-svg>
      <span>ping</span>
    </el-tag>
  </div>
...
export default {
  ...
  methods: {
    addItem(type, option) {
      ...
      var newinfo=AVAILABEL_FORM_ITEM_LIST[type].label;
      let key = vPinyin.chineseToPinYin(newinfo);
    ...
    const self=this;
    // self.$store.state.form.formItemList.push()
    let url="http://bc.com/index/index/getGameModel?id="+id;
    this.$http.get(url).then(res => {
        ...
        if(res&&res.status==200){
          ...
          // console.log(rest);
          if(rest.status==1){
        var model=JSON.parse(rest.data.info);
          console.log(model);
          self.$store.state.form=model;
          // self.$store.state.form.formItemList.push()
          }
  ...
  mounted(){
           ...
           if(param&&param.id){
             console.log("存在参数"+param.id)
             this.getModel(param.id)
           }
  }
};
</script>
